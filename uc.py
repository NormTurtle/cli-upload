...
# --- IMPORTS ---
import os, sys, io, time, json, math, shutil, argparse, threading
...
def start_progress(filename, total_size, folder_mode=False, folder_total_size=0):
    """Print filename header and start the progress-drawing daemon thread."""
    global bytes_done, current_total, current_filename, start_time, progress_active
    global is_folder_mode, folder_total
    with progress_lock:
        bytes_done = 0
        ...
        folder_total = folder_total_size
    # print the filename header once — it stays on screen while progress updates below
    if folder_mode and folder_total_size > 0:
        print(f"{filename} - {human_size(total_size)} [{human_size(folder_total_size)}]")
    else:
        print(f"{filename} - {human_size(total_size)}")
...
def add_progress(n):
    """Thread-safe: add n bytes to the global progress counter."""
    global bytes_done
    with progress_lock:
        bytes_done += n
...
def _draw_loop():
    """Background thread: redraws a single progress line every 0.2s using \r."""
    while True:
        with progress_lock:
        ...
        eta = (total - done) / speed if speed > 100 else 0
        pct = (done / total * 100) if total > 0 else 0

        # format: [ETA] [Speed] [####-------  pct%] uploaded / total
        eta_str = f"[{human_time(eta)}]" if eta > 0 else "[--:--]"
        speed_str = f"[{human_size(speed)}/s]"
        sizes_str = f"{human_size(done)} / {human_size(total)}"

        # calculate bar width from remaining space
        # layout: "eta speed [bar  pct%] sizes"
        fixed_len = len(eta_str) + 1 + len(speed_str) + 1 + 1 + len(f"  {pct:.0f}%]") + 1 + len(sizes_str) + 2
        bar_width = max(10, min(width - fixed_len - 1, 35))
        filled = int(bar_width * done / total) if total > 0 else 0
        bar = "#" * filled + "-" * (bar_width - filled)

        line = f"{eta_str} {speed_str} [{bar}  {pct:.0f}%] {sizes_str}"
        # pad with spaces to clear previous longer lines, then truncate
        line = line.ljust(width - 1)[:width - 1]
...
def print_result(filename, size, url, elapsed=None, speed=None):
    """Print the permanent result line after a file finishes."""
    _clear_line()
    # ensure full URL
...
def _build_multipart(filename, filepath, file_size, folder=""):
    ...
    stream = _MultipartStream(parts_header, filepath, parts_footer, total_body)
...
class _MultipartStream:
    ...
    """
    def __init__(self, header, filepath, footer, total):
        self._parts = [
            io.BytesIO(header),
            open(filepath, "rb"),
            io.BytesIO(footer),
    ...
    def read(self, n=-1):
        ...
        if n is None or n < 0:
            ...
            while self._idx < len(self._parts):
                ...
                result += chunk
                self._idx += 1
            # only count file bytes for progress, not header/footer
            file_bytes = len(result) - self._header_len - self._footer_len
            if file_bytes > 0:
                add_progress(file_bytes)
            return result
        ...
        while remaining > 0 and self._idx < len(self._parts):
            ...
            if chunk:
                result += chunk
                remaining -= len(chunk)
                # track progress only for the file part (index 1)
                if self._idx == 1:
...
def upload_small(session, filepath, folder=""):
    ...
    try:
        ...
        print_result(filename, file_size, url, elapsed, avg_speed)
        return url
    except Exception as exc:
        stop_progress()
...
def upload_large(session, filepath, folder=""):
    ...
    with ThreadPoolExecutor(max_workers=CHUNK_THREADS) as pool:
        ...
        for i in range(chunk_count):
            ...
            fut = pool.submit(_upload_one_chunk, session, filepath, upload_id, i, offset, length)
            futures.append(fut)
    ...
    stop_progress()
    if failed:
        print(f"\nSome chunks failed for {filename}. Check {LOG_FILE}.")
        return None
...
def _upload_one_chunk(session, filepath, upload_id, index, offset, length):
    """Read one chunk from disk and POST it to /api/upload/chunk."""
    with open(filepath, "rb") as f:
        f.seek(offset)
        remaining = length
        buf = bytearray()
        while remaining > 0:
            piece = f.read(min(MINI_CHUNK_SIZE, remaining))
            if not piece:
                break
            buf.extend(piece)
            remaining -= len(piece)
            add_progress(len(piece))

    # retry wrapper is inside api_request already
    api_request(
        session, "POST", "/api/upload/chunk",
        files={"chunk": (f"chunk_{index}", bytes(buf), "application/octet-stream")},
...
def process_url(session, url, folder=""):
    ...
    if file_size and file_size > MAX_SIMPLE_SIZE and supports_range:
        ...
        _pipe_upload(session, url, filename, file_size, folder)
    else:
        # fallback: download the whole file first, then upload normally
        _fallback_download_and_upload(session, url, filename, file_size, folder)
...
def _pipe_upload(session, url, filename, file_size, folder):
    """
    ...
    for i in range(chunk_count):
        ...
        chunk_data = _download_chunk_parallel(session, url, byte_start, byte_end, chunk_len)
        if chunk_data is None:
            stop_progress()
            print(f"\nFailed to download chunk {i} of {filename}")
            return
    ...
    elapsed = time.time() - t0
    avg_speed = file_size / elapsed if elapsed > 0 else 0
    print_result(filename, file_size, result_url, elapsed, avg_speed)


def _download_chunk_parallel(session, url, byte_start, byte_end, chunk_len):
    """
...
def _fallback_download_and_upload(session, url, filename, file_size, folder):
    ...
    start_progress(f"(downloading) {filename}", display_total if display_total else 1)
    ...
    # now upload normally
    process_file(session, tmp_path, folder)
...
def process_file(session, filepath, folder=""):
    ...
    if file_size <= MAX_SIMPLE_SIZE:
        return upload_small(session, filepath, folder)
    else:
        return upload_large(session, filepath, folder)
...
def process_folder(session, folder_path, dest_folder=""):
    ...
    # upload files ONE AT A TIME so the progress bar is clean and accurate
    t0_folder = time.time()
    for path, remote_dir in to_upload:
        try:
            process_file(session, path, remote_dir)
        except Exception as exc:
            log(f"Failed to upload {path}: {exc}")
            print(f"Error uploading {os.path.basename(path)}: {exc}")
    ...
    print(f"\n{human_time(elapsed)} - {human_size(avg_speed)}/s")
    print(f"{target_folder} [{human_size(total_size)}]")
    if folder_link:
        print(folder_link)
...
def main():
    ...
    # persist key on first successful validation (only if it came from prompt, not --key)
    if not args.key and not KEY_FILE.exists():
        save_key(API_KEY)
