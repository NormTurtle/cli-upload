...
class _MultipartStream:
        ...
        while remaining > 0 and self._idx < len(self._parts):
            ...
            if chunk:
                ...
                if prog > 0:
                    ...
                    if self._progress_buffer >= 1024 * 1024:
                        self._flush_progress()
            else:
                self._idx += 1
        return result
    ...
    def seek(self, offset):
        ...
        if file_progress_reported > 0:
            flushed_progress = file_progress_reported - self._progress_buffer
            if flushed_progress > 0:
                add_progress(self._filename, -flushed_progress)
        self._progress_buffer = 0
        self._bytes_read = 0
