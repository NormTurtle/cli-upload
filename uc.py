...
class _MultipartStream:
    ...
    def __init__(self, header, file_obj, footer, total, file_size, filename):
        ...
        self._bytes_read = 0
        self._progress_buffer = 0  # ⚡ Bolt: Buffer progress updates to reduce thread lock contention

    def _flush_progress(self):
        """Flush the buffered progress to the global tracker to reduce lock contention."""
    ...
    def seek(self, offset):
        ...
        if file_progress_reported > 0:
            reported = file_progress_reported - self._progress_buffer
            if reported > 0:
                add_progress(self._filename, -reported)
        self._progress_buffer = 0
        self._bytes_read = 0
